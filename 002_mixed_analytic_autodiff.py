...
env.new("end", "Marker", at=10.0, from_="qd@end")
(env.new("bendh", "Bend", angle="bphi", k0_from_h=True, length=1.0),)
(
    env.new(
        "bendv", "Bend", angle="bphi", rot_s_rad=np.pi / 2, k0_from_h=True, length=1.0
    ),
)
env.new("qq", "Quadrupole", k1=0.0, length=1.0, anchor="start")

line = env.new_line(
    components=[
        env.place("qf", anchor="start", at=0.0),
        env.place("drift", at=1.0, from_="qf@end"),
        env.place("bendh", anchor="start", at=5.0, from_="drift@end"),
        # env.place('bendv', anchor='start', at=18., from_='bendh@end'),
        env.place("qd", anchor="start", at=10.0, from_="qf@end"),
        env.place("drift2", anchor="start", at=11.0, from_="qd@end"),
        env.place("qq", anchor="start", at=12.0, from_="drift2@end"),
        env.place("end", at=10.0, from_="drift@end"),
    ]
)
    ...
    f_matrix = jnp.array(
        [
            ...
            [
                ...
                0,
                1,
                l / (beta0**2 * gamma0**2) - h**2 / beta0**2 * j1,
            ],
            [0, 0, 0, 0, 0, 1],
        ]
    )
...
def get_values_from_transfer_matrix(transfer_matrix, tw0):
    betx = (
        ...
        / tw0.betx[0]
        * (
            (transfer_matrix[0, 0] * tw0.betx[0] - transfer_matrix[0, 1] * tw0.alfx[0])
            ** 2
            + transfer_matrix[0, 1] ** 2
        )
    )
    bety = (
        1
        / tw0.bety[0]
        * (
            (transfer_matrix[2, 2] * tw0.bety[0] - transfer_matrix[2, 3] * tw0.alfy[0])
            ** 2
            + transfer_matrix[2, 3] ** 2
        )
    )
    alfx = (
        -1
        ...
        * (
            (transfer_matrix[0, 0] * tw0.betx[0] - transfer_matrix[0, 1] * tw0.alfx[0])
            * (
                transfer_matrix[1, 0] * tw0.betx[0]
                - transfer_matrix[1, 1] * tw0.alfx[0]
            )
            + transfer_matrix[0, 1] * transfer_matrix[1, 1]
        )
    )
    alfy = (
        -1
        ...
        * (
            (transfer_matrix[2, 2] * tw0.bety[0] - transfer_matrix[2, 3] * tw0.alfy[0])
            * (
                transfer_matrix[3, 2] * tw0.bety[0]
                - transfer_matrix[3, 3] * tw0.alfy[0]
            )
            + transfer_matrix[2, 3] * transfer_matrix[3, 3]
        )
    )
    mux = tw0.mux[0] + jnp.arctan2(
        transfer_matrix[0, 1],
        transfer_matrix[0, 0] * tw0.betx[0] - transfer_matrix[0, 1] * tw0.alfx[0],
    ) / (2 * jnp.pi)
    muy = tw0.muy[0] + jnp.arctan2(
        transfer_matrix[2, 3],
        transfer_matrix[2, 2] * tw0.bety[0] - transfer_matrix[2, 3] * tw0.alfy[0],
    ) / (2 * jnp.pi)
    dx = (
        transfer_matrix[0, 0] * tw0.dx[0]
        + transfer_matrix[0, 1] * tw0.dpx[0]
        + transfer_matrix[0, 5]
    )
    dy = (
        transfer_matrix[2, 2] * tw0.dy[0]
        + transfer_matrix[2, 3] * tw0.dpy[0]
        + transfer_matrix[2, 5]
    )
    dpx = (
        transfer_matrix[1, 0] * tw0.dx[0]
        + transfer_matrix[1, 1] * tw0.dpx[0]
        + transfer_matrix[1, 5]
    )
    dpy = (
        transfer_matrix[3, 2] * tw0.dy[0]
        + transfer_matrix[3, 3] * tw0.dpy[0]
        + transfer_matrix[3, 5]
    )
...
def get_values_new_from_transfer_matrix(r_mat, param_values):
    ...
    betx = (
        ...
        / param_values[0]
        * (
            (r_mat[0, 0] * param_values[0] - r_mat[0, 1] * param_values[2]) ** 2
            + r_mat[0, 1] ** 2
        )
    )
    bety = (
        ...
        / param_values[1]
        * (
            (r_mat[2, 2] * param_values[1] - r_mat[2, 3] * param_values[3]) ** 2
            + r_mat[2, 3] ** 2
        )
    )
    alfx = (
        ...
        / param_values[0]
        * (
            (r_mat[0, 0] * param_values[0] - r_mat[0, 1] * param_values[2])
            * (r_mat[1, 0] * param_values[0] - r_mat[1, 1] * param_values[2])
            + r_mat[0, 1] * r_mat[1, 1]
        )
    )
    alfy = (
        ...
        / param_values[1]
        * (
            (r_mat[2, 2] * param_values[1] - r_mat[2, 3] * param_values[3])
            * (r_mat[3, 2] * param_values[1] - r_mat[3, 3] * param_values[3])
            + r_mat[2, 3] * r_mat[3, 3]
        )
    )
    mux = param_values[4] + jnp.arctan2(
        r_mat[0, 1], r_mat[0, 0] * param_values[0] - r_mat[0, 1] * param_values[2]
    ) / (2 * jnp.pi)
    muy = param_values[5] + jnp.arctan2(
        r_mat[2, 3], r_mat[2, 2] * param_values[1] - r_mat[2, 3] * param_values[3]
    ) / (2 * jnp.pi)
...
def derive_values_by_backtrack(elements, tw0):
    ...
    for elem in elements:
        if isinstance(elem, xt.Quadrupole):
            transfer_matrix = get_transfer_matrix_quad(
                elem.k1,
                elem.length,
                tw0.particle_on_co.beta0[0],
                tw0.particle_on_co.gamma0[0],
            )
            ...
            k0 = elem.k0 if not elem.k0_from_h else elem.h
            transfer_matrix = get_transfer_matrix_bend(
                k0,
                elem.k1,
                elem.length,
                elem.h,
            ...
            transfer_matrix = get_transfer_matrix_drift(
                elem.length, tw0.particle_on_co.beta0[0], tw0.particle_on_co.gamma0[0]
            )
            if np.any(np.isnan(transfer_matrix)):
                print("Turned to NaN in transfer matrix!!")
                print(f"Element: {elem}")
...
def compute_param_derivatives(line, tw0):
    def get_values(k1_arr):
        ...
        assert len(k1_arr) == len(
            [elem for elem in line.elements if isinstance(elem, xt.Quadrupole)]
        )
    ...
    return jax.jacrev(get_values)(
        jnp.array(
            [elem.k1 for elem in line.elements if isinstance(elem, xt.Quadrupole)]
        )
    )
...
def get_values_from_transfer_matrix_sp(transfer_matrix, tw0):
    ...
    mux = tw0.mux[0] + sp.atan(
        transfer_matrix[0, 1]
        / (transfer_matrix[0, 0] * tw0.betx[0] - transfer_matrix[0, 1] * tw0.alfx[0])
    ) / (2 * sp.pi)
    muy = tw0.muy[0] + sp.atan(
        transfer_matrix[2, 3]
        / (transfer_matrix[2, 2] * tw0.bety[0] - transfer_matrix[2, 3] * tw0.alfy[0])
    ) / (2 * sp.pi)
...
def compute_beta_derivative_sym(line, tw0):
    quadrupoles = [
        line.elements[i]
        for i in range(len(line.elements))
        if isinstance(line.elements[i], xt.Quadrupole)
    ]
...
(
    ...
    parameter_values,
    parameter_matrix,
) = derive_values_by_backtrack(line, tw0)
...
print(
    tabulate(
        ...
        tablefmt="fancy_grid",
        headers=["Parameter", "Twiss", "Backtracked"],
    )
)
