...
env.new('dquad', 'Multipole', knl=[0., 'dk1l']),

line = env.new_line(components=[
    env.place('dquad', anchor='start', at=0.),
...
env['dk1l'] += eps
line.twiss4d(init=tw0, start=xt.START, end=xt.END)
...
def derive_beta_by_backtrack(line, tw0):
    ...
    for elem in line.elements:
        if isinstance(elem, xt.Quadrupole):
            ...
            print(f"Ignoring element of type {type(elem)}")
            continue
...
def compute_beta_derivative_sym(line, tw0):
    k1_vars = [sp.Symbol(f'k1_{i}') for i, elem in enumerate(line.elements) if isinstance(elem, xt.Quadrupole) or isinstance(elem, xt.Multipole)]
...
print("Finite difference betx: ", fd_dict['betx'])
print("Twiss param derivative: ", tw0.get_twiss_param_derivative(src='dquad', observation='_end_point')['dbetx'])
